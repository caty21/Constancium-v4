  ...
  return (
    <header className="fixed top-0 w-full z-50">
      ...
      <div className="border-b border-white/5 hidden md:block bg-[#0a1020]/80">
        <div className="max-w-7xl mx-auto px-6 md:px-8 h-9 flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            <a href="tel:+33767694984" className="flex items-center gap-1.5 text-white/40 hover:text-[#D4AF37] text-xs transition-colors">
              <Phone className="h-3 w-3" />
              07 67 69 49 84
            </a>
            <div className="w-px h-3 bg-white/15" />
            <a href="mailto:capucine@constancium.com" className="flex items-center gap-1.5 text-white/40 hover:text-[#D4AF37] text-xs transition-colors">
      ...
      <div className="bg-[#0F1729]/70 backdrop-blur-md">
          ...
          <div className="flex items-center justify-between h-[70px]">
            <Link href="/">
              <div className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="font-serif text-xl font-bold text-white tracking-tight group-hover:text-[#D4AF37] transition-colors" data-testid="text-logo">
                  Constancium
                </span>
              </div>
            </Link>
            ...
            <nav className="hidden lg:flex items-center gap-0.5">
              {navItems.map((item) => (
                <div key={item.label} className="relative group">
                  <a
                    href={item.href}
                    className="relative px-4 py-2 text-sm font-medium text-white/70 hover:text-white transition-colors inline-flex items-center gap-1 group"
                  ...
                  {item.submenuType === "gamme" && (
                    ...
                    >
                      <div className="bg-[#0F1729] border border-white/10 rounded-xl shadow-2xl min-w-[520px] overflow-hidden">
                        <div className="flex">
                          <div className="w-1/2 border-r border-white/5 py-3">
                            ...
                            {gammeCategories.map((category, idx) => {
                              ...
                              return (
                                <a
                                  ...
                                  className={`flex items-center gap-3 px-4 py-2.5 transition-all ${hoveredCategory === idx ? "bg-white/5" : "hover:bg-white/5"}`}
                                  onMouseEnter={() => setHoveredCategory(idx)}
                                  data-testid={`link-gamme-${category.id}`}
                                >
                                  <div className={`p-1.5 rounded-md transition-colors ${hoveredCategory === idx ? "bg-[#D4AF37]/25" : "bg-[#D4AF37]/10"}`}>
                                    <CategoryIcon className="h-3.5 w-3.5 text-[#D4AF37]" />
                                  </div>
                                  <span className={`text-sm font-medium transition-colors ${hoveredCategory === idx ? "text-white" : "text-white/60"}`}>
                                    {category.title}
                                  </span>
                                  <ChevronRight className={`h-3.5 w-3.5 ml-auto transition-colors ${hoveredCategory === idx ? "text-[#D4AF37]" : "text-white/20"}`} />
                                </a>
                              );
                            })}
                          </div>
                          <div className="w-1/2 py-3 bg-white/[0.02]">
                            <div className="px-4 pb-2 mb-1 border-b border-white/5">
                              <span className="text-[10px] font-semibold text-[#D4AF37]/60 uppercase tracking-widest">
                            ...
                            {hoveredCategory !== null ? (
                              gammeCategories[hoveredCategory].subcategories.map((sub) => (
                                <a
                                  ...
                                  className="block px-4 py-2 text-sm text-white/50 hover:text-[#D4AF37] transition-colors"
                              ...
                              <div className="px-4 py-4 text-sm text-white/20 italic">Survolez une catégorie</div>
                            )}
                          </div>
                        </div>
                      </div>
                  ...
                  {item.submenuType === "simulateur" && (
                    <div className="absolute top-full left-0 pt-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none group-hover:pointer-events-auto">
                      <div className="bg-[#0F1729] border border-white/10 rounded-xl shadow-2xl py-2 min-w-[300px]">
                        <div className="px-4 pb-2 mb-1 border-b border-white/5">
                        ...
                        {simulatorSubmenu.map((subItem) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div>
                                <span className="block text-sm font-medium text-white/70 group-hover/item:text-white transition-colors">
                                  {subItem.label}
                                </span>
                                <span className="text-xs text-white/35">{subItem.description}</span>
                              </div>
                            </a>
                          );
                        })}
        ...
        {/* Mobile menu */}
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-white/10 bg-[#0F1729]">
            <nav className="py-3 px-4 space-y-1">
              {navItems.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="block px-3 py-3 text-white/70 hover:text-white text-sm font-medium hover:bg-white/5 rounded-lg transition-colors"
